...
contract SyndicateGovernorTest is Test {
    ...
    function test_fullLifecycle_proposeVoteExecuteSettle() public {
        ...
        // 9. Cooldown — verify cooldownEnd is set
        assertGt(governor.getCooldownEnd(address(vault)), block.timestamp - COOLDOWN_PERIOD);

        // 10. Wait for cooldown to pass, then withdraw and execute new strategy
        vm.warp(governor.getCooldownEnd(address(vault)) + 1);

        vm.prank(lp1);
        vault.withdraw(10_000e6, lp1, lp1);

        uint256 proposalId2 = _createAndExecuteProposal(1500, 7 days);
        assertEq(governor.getActiveProposal(address(vault)), proposalId2);
    }
    ...
    function test_fullLifecycle_emergencySettle() public {
        ...
        // Owner emergency settles with custom unwind calls
        BatchExecutorLib.Call[] memory customCalls = new BatchExecutorLib.Call[](1);
        customCalls[0] = BatchExecutorLib.Call({
            target: address(usdc), data: abi.encodeCall(usdc.approve, (address(targetToken), 0)), value: 0
        });
        ...
        // Agent still gets fee on profit from emergency settle
    ...
    function test_fullLifecycle_multipleProposalsSequential() public {
        // First strategy
        uint256 pid1 = _createAndExecuteProposal(1500, 3 days);
        ...
        // Second strategy
        uint256 pid2 = _createAndExecuteProposal(2000, 5 days);
