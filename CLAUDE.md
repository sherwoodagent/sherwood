...
- SyndicateGovernor runtime is **23,811 / 24,576 bytes (765-byte EIP-170 margin)** as of 2026-04-22 post-V1.5 — the timelock removal (see Governor Key Concepts below) reclaimed 693 bytes while still adding `guardianFeeBps` + `guardianFeeRecipient`. Setters now apply immediately (owner is a multisig with its own delay). Run `forge build --sizes` before any governor edit.
- GuardianRegistry runtime is **24,492 / 24,576 bytes (84-byte EIP-170 margin)** as of 2026-04-22 post-V1.5. Tight after adding delegation (~2.5k) + commission (+900) + guardian-fee claim paths (+1.4k). Reclaim levers used to stay under EIP-170: dropped `nonReentrant` where CEI is respected, dropped external `pending*` / `proposalGuardianPool` views (UIs static-call instead), dropped dedicated `ApproverFeeFlushed` event (observable via ERC20 Transfer + `unclaimedApproverFee` zeroing).
